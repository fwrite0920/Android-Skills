...
- 先填写 Required Inputs（目标、时限、风险级别）
...
## Workflow
1. 先确认 Required Inputs（目标、现况、限制条件）
2. 在 Scenario Router 找到最接近的情境
3. 依顺序加载对应技能并运行
4. 回到 Skill Dependency Graph 检查漏项，并确认每个技能 Gate 已执行
...
- `建议技能组合`（2-3 个，按顺序）
- `执行顺序`（先后与依赖关系）
- `验收口径`（对应各技能 Quick Checklist + Gate）
- `遗漏检查`（Skill Dependency Graph 回查结果）

## Routing Gate (验收门槛)

- 每个被选技能都要填写 `Required Inputs`
- 每个被选技能都要执行对应 `* Gate`
- 输出最终验收结论：通过 / 阻挡项 / 后续动作

## Quick Checklist

- [ ] Required Inputs 已填写并冻结（目标/限制/风险）
- [ ] 已从 Scenario Router 选定最小技能组合
- [ ] 已确认组合顺序与依赖关系
- [ ] 已检查每个技能 Gate 是否执行
- [ ] 已回查 Skill Dependency Graph 无关键漏项
...
```
1. observability_first      → 先定义输入（SLO/owner/告警渠道）再建指标闭环
...
2. **填写 Required Inputs**：先把目标、限制、风险写清楚
3. **依序运行技能**：按照建议顺序阅读并应用各技能
4. **执行各技能 Gate**：例如 Style/Delivery/Release/Monitoring Gate
5. **参考 Checklist**：每个技能都附有 Quick Checklist 供 Code Review 使用
